...
export async function GET(req: Request, { params }: { params: { articleId: string } }) {
  try {
    ...
    const [article] = await db.select().from(articleTable).where(eq(articleTable.id, articleId))
...
export async function DELETE(req: Request, { params }: { params: { articleId: string } }) {
  try {
    ...
    if (!deletedArticle) {
      return NextResponse.json({ error: "Article not found or you do not have permission to delete." }, { status: 404 })
    }

    return NextResponse.json({ message: "Article deleted successfully" }, { status: 200 })
  } catch (error) {
    console.error("❌ [ARTICLE_DELETE_API] Error:", error)
    return NextResponse.json({ error: "Internal Server Error" }, { status: 500 })
  }
}
