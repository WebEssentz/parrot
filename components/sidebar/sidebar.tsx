...
// --- Child components are unchanged except for how they receive props ---
const SearchInput = ({ shouldShowContent }: { shouldShowContent: boolean }) => { // Added prop
  return (
    <div className="px-2">
      <motion.button
        layout
        transition={{ type: "tween", ease: "easeInOut", duration: 0.3 }}
        className={clsx("h-10 w-full group flex items-center overflow-hidden rounded-full relative", "cursor-pointer border text-zinc-500", "bg-bg-[#fcfcfc] border-zinc-200 hover:bg-zinc-200 hover:border-zinc-300", "dark:bg-zinc-700/20 dark:border-zinc-700/75 dark:text-zinc-400 dark:hover:bg-[#3b3b3b] dark:hover:border-zinc-700", { "w-full px-3": shouldShowContent, "w-10 justify-center": !shouldShowContent, })}> {/* Used shouldShowContent */}
        <div className="w-full flex items-center justify-center">
          <AnimatePresence>
            {shouldShowContent && (<motion.span key="search-text" layout={false} initial={{ opacity: 0 }} animate={{ opacity: 1, transition: { delay: 0.2, duration: 0.2 } }} exit={{ opacity: 0, transition: { duration: 0.1 } }} className="mr-auto text-sm whitespace-nowrap">Search...</motion.span>)} {/* Used shouldShowContent */}
          </AnimatePresence>
          <Search size={17} />
        </div>
      </motion.button>
...
const NewChatButton = ({ shouldShowContent }: { shouldShowContent: boolean }) => { // Added prop
  const { isDesktopSidebarCollapsed, toggleSidebar, isSidebarOpen } = useSidebar() // Keep this for existing logic
  ...
  return (
    <div className="px-2">
      <button onClick={handleNewChat} className={clsx("h-10 w-full group flex items-center justify-start px-3 overflow-hidden relative", "border border-transparent text-zinc-500 dark:text-zinc-300 cursor-pointer", "rounded-lg hover:text-primary hover:bg-zinc-200/70 dark:hover:bg-zinc-700/30", "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring", { "justify-start px-3 -ml-[0.2rem]": shouldShowContent, "justify-center pl-[0.57rem]": !shouldShowContent, })}> {/* Used shouldShowContent */}
        <NewChatIcon className="flex-shrink-0" />
        <AnimatePresence>
          {shouldShowContent && (<motion.span key="new-chat-text" initial={{ opacity: 0 }} layout={false} animate={{ opacity: 1, transition: { delay: 0.1, duration: 0.2 } }} exit={{ opacity: 0, transition: { duration: 0.1 } }} className="text-sm whitespace-nowrap ml-2 -mt-[0.2rem]">New chat</motion.span>)} {/* Used shouldShowContent */}
        </AnimatePresence>
      </button>
    </div>
  )
...
export const Sidebar = () => {
  ...
  // This effect will ensure the sidebar is open on mobile initially if not already
  useEffect(() => {
    if (hasMounted && !initialLoadHandled.current) {
      if (!isDesktop && !isSidebarOpen) {
        // Toggle the mobile sidebar to open it on initial load if on mobile
        toggleSidebar(); 
      }
  ...
  const handleSidebarClick = () => {
    ...
    // No need to toggle mobile sidebar here, as it's handled by the `isSidebarOpen` state
    // that should be controlled by a separate hamburger menu button.
  }
  ...
  // Determine if content should be shown based on desktop collapse or mobile open state
  ...
  return (
    <motion.aside
      ...
      layout
      initial={false}
      // Only animate based on the collapse state if on desktop
      animate={isDesktop && isDesktopSidebarCollapsed ? "collapsed" : "open"}
      // Only provide collapse/open width variants on desktop
      ...
      className={clsx(
        ...
        { 
          // On desktop, fixed positioning and specific background based on collapsed state
          ...
          // On mobile, always take full width if open, and specific background
          "w-64 bg-[#f9f9f9] dark:bg-[#1E1E1E] dark:border-r-[#333333]": !isDesktop && isSidebarOpen,
          // Hide sidebar completely when not desktop and not open (assuming a slide-in/out mobile menu)
      ...
      <div className="flex flex-col h-full w-full">
        <div onClick={(e) => e.stopPropagation()}>
          <div className="flex-shrink-0 p-2 min-h-[56px] w-full flex items-center relative">
            <AnimatePresence>
              {shouldShowContent && (<motion.div key="avurna-title" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20, transition: { duration: 0.1 } }} transition={{ duration: 0.3, ease: "easeOut" }} className="absolute left-4 select-none font-medium text-zinc-900 dark:text-white" style={{ fontSize: "20px", lineHeight: "22px", fontFamily: 'Google Sans, "Helvetica Neue", sans-serif', letterSpacing: "normal", }}>Avurna</motion.div>)} {/* Used shouldShowContent */}
            </AnimatePresence>
            {/* Modified the positioning of the toggle button */}
            <motion.div layout transition={{ type: "tween", ease: "easeInOut", duration: 0.3 }} className={clsx({ "absolute right-2 top-2": !shouldShowContent, "ml-auto": shouldShowContent })}> {/* Used shouldShowContent */}
              {/* The collapse/expand button is now a desktop-only feature */}
              {isDesktop && (
                <SidebarIconButton 
          ...
          {/* Pass shouldShowContent to child components */}
        ...
        <div className="flex-grow overflow-y-auto mt-4">
          <AnimatePresence>
            {shouldShowContent && (<motion.div key="chat-history-list" layout={false} initial={{ opacity: 0 }} animate={{ opacity: 1, transition: { delay: 0.25, duration: 0.2 } }} exit={{ opacity: 0, transition: { duration: 0.1 } }}><ChatHistoryList /></motion.div>)} {/* Used shouldShowContent */}
          </AnimatePresence>
        </div>
            ...
            <CustomUserMenu>
                <button className={clsx("user-profile-button w-full flex items-center p-2 rounded-lg", "cursor-pointer")}>
                    ...
                    <div className="flex items-center flex-grow min-w-0">
                      <UserAvatar />
                      <AnimatePresence>
                        {shouldShowContent && ( // Used shouldShowContent
