...
export const ChatHistoryList = () => {
  ...
  const { chats, isLoading, isError, mutateChats, hasMore, isLoadingMore, setSize, size } = useChats();
  ...
  // --- 3. THIS IS THE FIX ---
  // The error state now includes a user-friendly message and a "Retry" button.
  if (isError) {
    return (
      <div className="p-4 mt-10 text-center text-sm">
        <p className="text-red-500">Failed to load chats.</p>
        <button
          onClick={() => mutateChats()} // Calling mutate will re-trigger the fetch
          className="mt-3 inline-flex items-center gap-2 px-3 py-1.5 rounded-md border border-zinc-300 dark:border-zinc-700 text-zinc-700 dark:text-zinc-300 hover:bg-primary dark:hover:bg-zinc-800 cursor-pointer"
        >
