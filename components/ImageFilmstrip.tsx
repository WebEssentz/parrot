      ...
      <div className="flex h-full w-full flex-col lg:flex-row">
        ...
        >
          ...
          <div className="hidden h-full w-full p-4 lg:block">
            ...
            <div className="relative h-full w-full">
              {imageFiles.map((image, index) => {
                ...
                return (
                  <motion.button key={image.id} onClick={() => setCurrentIndex(index)} animate={{ y, scale, zIndex }} transition={{ type: "spring", stiffness: 400, damping: 30 }} whileTap={{ scale: index === currentIndex ? 1 : 0.8 }} whileHover={{ scale: 1.05, zIndex: 50 }} className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 group w-full rounded-lg overflow-hidden border-2 ${index === currentIndex ? "border-blue-500 ring-2 ring-blue-500/30" : "border-zinc-700/60 hover:border-zinc-500"}`}>
          ...
          <div className="block h-full w-full p-2 lg:hidden">
            <div ref={mobileContainerRef} className="flex h-full items-center gap-3 overflow-x-auto scrollbar-hide">
              {imageFiles.map((image, index) => (
                <button key={image.id} ref={(el) => { mobileThumbRefs.current[index] = el }} onClick={() => setCurrentIndex(index)} className={`relative flex-shrink-0 w-24 h-16 rounded-md overflow-hidden border-2 transition-all duration-300 ${index === currentIndex ? "border-blue-500 scale-105" : "border-zinc-700/60 opacity-70"}`}>
        ...
        <div className="flex-1 flex items-center justify-center p-2 pt-8 lg:p-8 min-h-0" onClick={(e) => e.stopPropagation()}>
          <div className="relative w-full h-full flex items-center justify-center">
            ...
            {imageFiles.length > 1 && (
              <>
                <button onClick={goToPrevious} className="absolute left-2 top-1/2 -translate-y-1/2 w-9 h-9 bg-black/50 hover:bg-black/70 rounded-full flex items-center justify-center text-white transition-all lg:w-11 lg:h-11"><ChevronLeft size={20} /></button>
                <button onClick={goToNext} className="absolute right-2 top-1/2 -translate-y-1/2 w-9 h-9 bg-black/50 hover:bg-black/70 rounded-full flex items-center justify-center text-white transition-all lg:w-11 lg:h-11"><ChevronRight size={20} /></button>
              </>
            )}
        ...
        {/* Close Button */}
        <button onClick={onClose} className="absolute top-4 right-4 w-9 h-9 bg-black/50 hover:bg-black/70 rounded-full flex items-center justify-center text-white transition-all z-20 lg:w-11 lg:h-11"><X size={20} /></button>
      </div>
    </motion.div>
  )
}