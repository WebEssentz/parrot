...
// // Optionally import OpenAI fallback (if available)
// let openai: any = undefined;
// let openaiGenerateText: any = undefined;
// try {
//   openai = require('@ai-sdk/openai');
//   openaiGenerateText = require('ai').generateText;
// } catch {}
// // --- GEMINI API CALL WITH ROBUST RETRY (gemini-2.0-flash as fallback) ---
// async function robustGeminiCall({ prompt, modelName = 'gemini-2.5-flash-preview-04-17', maxRetries = 5 }: {
//   prompt: string;
//   modelName?: string;
//   maxRetries?: number;
...
//     } catch (e: any) {
//       lastError = e;
//       if (e && e.statusCode && e.statusCode < 500) break;
//     }
//     await new Promise(res => setTimeout(res, delay));
//     delay = Math.min(delay * 2, 4000);
//   }
//   // Fallback to gemini-2.0-flash if not already tried
...
//     }
//   }
//   return { text: '', error: lastError ? (lastError.message || String(lastError)) : 'Unknown Gemini API error' };
// }
// import fs from 'fs';
...
//   const { text, error } = await robustGeminiCall({ prompt });
//   let parsed: any = {};
//   try {
//     parsed = JSON.parse(text);
//   } catch {
//     parsed = { useVision: false, reason: text || error };
//   }
//   return parsed;
// }
...
//     const { text, error } = await robustGeminiCall({ prompt: prompt + `\n[Image base64]\n` + screenshotBase64 });
//     let parsed: any = {};
//     try {
//       parsed = JSON.parse(text);
//     } catch {
//       parsed = { visualType: 'unknown', description: text || error, confidence: 0 };
//     }
...
//   const { text, error } = await robustGeminiCall({ prompt: planPrompt });
//   try {
...
//         const { text } = await robustGeminiCall({ prompt: checkPrompt });
